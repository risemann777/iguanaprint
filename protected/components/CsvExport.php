...
class CsvExport {
    ...
    */
    public static function export_one($rows, $coldefs, $boolPrintRows=true, $csvFileName=null, $separator=';')
    {
        $endLine = PHP_EOL;
        $returnVal = '';
        ...
        foreach($rows as $row){
            ...
            foreach($coldefs as $col=>$config){
                ...
                if(isset($row[$col])){
                    ...
                    $r .= '="'.iconv("utf-8", "windows-1251", $val).'"'.$separator;
                }
            }
    ...
    public static function export($firstline, $rows, $csvFileName=null, $separator=';')
    {
        $endLine = PHP_EOL;
        $returnVal = '';
        ...
        if($firstline){
            ...
            foreach($firstline as $col){
                $names .= iconv("utf-8", "windows-1251", $col).$separator;
            }
            $names = rtrim($names,$separator);
            $returnVal .= $names.$endLine;
        }
        ...
        foreach($rows as $row){
            $new_row = '';
            foreach($row as $item){
                $new_row .= iconv("utf-8", "windows-1251", $item).$separator;
            }
            $new_row = rtrim($new_row,$separator);
            $returnVal .= $new_row.$endLine;
        }
